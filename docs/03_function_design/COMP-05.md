...
COMP-05が持つ6関数を、種別ごとに3グループへ分けて示す。グループ構成は次のとおり: フィクスチャ全体のライフサイクル管理2件（FUNC-43〜44）／個々のテストのライフサイクル管理2件（FUNC-45〜46）／シナリオテストメソッド2件（FUNC-47〜48）。
