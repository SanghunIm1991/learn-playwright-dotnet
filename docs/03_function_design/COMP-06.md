...
| v1.6 | 2026-09-24 | レビュー指摘6件対応（教材`docs/tutorial/`を正として整合）。(1) 重大: v1.4で追加済みとしていたFUNC-49「サンプルアプリ（リポジトリ）の準備」が本文に無かったため、教材0章手順5（5-1取得〜5-4任意の完成版テスト実行）を手順5として追加し、目的・3.4節・到達経路図・6.3節にも反映（旧手順5「期待される出力例」は手順6へ繰り下げ）。(2) FUNC-49手順3を教材どおり`install chromium`とし、`pwsh`が無い場合の`powershell -ExecutionPolicy Bypass -File ...`代替を追記、太字見出し＋箇条書き形式に統一。(3) FUNC-54の「FUNC-48を引用しつつ解説」を、Cookieが保持される理由を教材自身の言葉で説明する記述に修正。(4) FUNC-55手順5のFUNC-39表記を統一。(5) 6.6節に「観点」行を追加 | ClaudeCode |
...
```mermaid
flowchart LR
    A["FUNC-49<br/>0. 環境構築・サンプルアプリの準備<br/>(コマンド操作あり)"] --> B["FUNC-50<br/>1. ベストプラクティス解説<br/>(概念解説のみ・コマンド操作なし)"]
