...
| v1.6 | 2026-09-24 | レビュー指摘6件対応（教材`docs/tutorial/`を正として整合）。(1) 重大: v1.4で追加済みとしていたFUNC-49「サンプルアプリ（リポジトリ）の準備」が本文に無かったため、教材0章手順5（5-1取得〜5-4任意の完成版テスト実行）を手順5として追加し、目的・3.4節・到達経路図・6.3節にも反映（旧手順5「期待される出力例」は手順6へ繰り下げ）。(2) FUNC-49手順3を教材どおり`install chromium`とし、`pwsh`が無い場合の`powershell -ExecutionPolicy Bypass -File ...`代替を追記、太字見出し＋箇条書き形式に統一。(3) FUNC-54の「FUNC-48を引用しつつ解説」を、Cookieが保持される理由を教材自身の言葉で説明する記述に修正。(4) FUNC-55手順5のFUNC-39表記を統一。(5) 6.6節に「観点」行を追加。(6) FUNC-49手順2に初回実行時メッセージ（テレメトリ・HTTPS開発証明書、`--trust`不要）の説明を追加 | ClaudeCode |
...
- **FUNC-49の役割**: FUNC-49（環境構築）は、この終点に到達するために必要な環境（.NET 10 SDK・NUnitプロジェクト・Playwrightブラウザ本体）と、FUNC-51でテスト対象として起動するサンプルアプリ（リポジトリの取得・ビルド・起動確認）を過不足なく整える内容とする。コマンドの実行結果（期待される出力例）を明記することで、初心者が各手順の成否を自己判断できるようにする（詳細は4節FUNC-49参照）。
...
- **目的**: Playwright未経験の初心者が、本教材を進めるために必要な開発環境（.NET 10 SDK、NUnitベースのPlaywrightテストプロジェクト、Playwrightブラウザ本体）と、テスト対象のサンプルアプリ（リポジトリ）を、迷わず・過不足なく整えられるようにする。
- **扱う内容**:
  1. **.NET 10 SDKの導入確認**
  ...
  2. **学習用プロジェクトの作成**
     ...
     - `dotnet`初回実行時に表示されるメッセージ（テレメトリ、HTTPS開発証明書の案内）の説明。サンプルアプリはHTTPのみで動くため`dotnet dev-certs https --trust`は実行不要である旨を明記する。
  ...
  3. **Playwrightブラウザ本体のインストール**
     - `dotnet build`を実行し、`bin/Debug/net10.0/playwright.ps1`スクリプトが生成されることを確認する手順（ビルド成功の表示を確認）。
     - `pwsh bin/Debug/net10.0/playwright.ps1 install chromium`でChromiumのみをインストールする手順。
     - `pwsh`（PowerShell 7）が無い環境向けの代替として、Windows標準のPowerShell 5.1で`powershell -ExecutionPolicy Bypass -File bin/Debug/net10.0/playwright.ps1 install chromium`を実行する手順。`-ExecutionPolicy Bypass`はその1回のプロセスに限った指定で、PC全体の設定は変更しない旨を説明する。
  4. **動作確認**
  ...
  5. **サンプルアプリ（リポジトリ）の準備**: 2章以降でテスト対象とするサンプルアプリを、学習者自身がビルド・起動できるようにする。
     - 5-1 取得: GitHubのリポジトリを`git clone`、または「Download ZIP」で取得する手順。置き場所は作業用フォルダ`C:\work`直下とし、学習用プロジェクト（`C:\work\MyFirstPlaywrightTests`）の中には置かない旨を注意として示す（中に置くとリポジトリ内の`.cs`ファイルまで学習用プロジェクトのビルド対象になりエラーとなるため）。ZIP展開時にフォルダが二重になる場合は内側（`global.json`がある側）で作業する旨も案内する。
     - 5-2 ビルド: リポジトリのルートフォルダで`dotnet build`を実行し、初回はNuGetパッケージがnuget.orgから自動で復元される旨を説明する。
     - 5-3 起動・画面確認: `dotnet run --project src/LearnPlaywright.Server`で起動し、`Now listening on: http://localhost:5080`等の表示を確認したうえで、ブラウザでフォーム画面を開いて送信・読み込みを試す手順。確認後は`Ctrl+C`で停止する（2章で改めて起動する）。
     - 5-4（任意）完成版テストの実行: 完成版テストプロジェクト用に`tests/LearnPlaywright.Tests/bin/Debug/net10.0/playwright.ps1 install chromium`でChromiumを導入し（`pwsh`が無い場合は手順3と同じ`powershell -ExecutionPolicy Bypass -File ...`の代替）、`dotnet test`を実行する手順。手動起動したサーバーが残っているとポート競合でE2Eテストが失敗するため停止してから実行する旨を明記する。
  6. **期待される出力例**: 各手順について、期待されるコンソール出力の例（バージョン文字列の表示形式、`Passed!`表示の形式、ビルド成功表示、サーバー起動時の待ち受けURL表示等）を示し、初心者が「うまくいったかどうか」を自己判断できるようにする。
- **対応するCOMP-01〜05の部分**:
  - 手順1〜4ではCOMP-01〜05の具体的な関数・コードは扱わない（環境そのものの構築が目的のため）。ただし手順3は、COMP-05関数設計書FUNC-43（`OneTimeSetUpAsync`）が前提とする「Playwrightブラウザ本体がインストール済みであること」という実行環境上の前提条件を、本章の時点で満たす位置づけを持つ。
  - 手順5はCOMP-01〜05を含むリポジトリ一式を取得・ビルド・起動するが、個々の関数・コードの解説は行わない（COMP-02サーバーの起動方法は2章FUNC-51の手動起動運用の予行となる）。
...
- **扱う内容**:
  1. ページの再読み込み（`page.ReloadAsync()`）でUIを初期状態に戻す手順（サンプルアプリが利用者をCookie〈`lp_user_id`〉で識別していること、同一ブラウザ内の再読み込みではCookieが保持されるため同じ利用者として読み込めること、新しく起動したブラウザにはCookieがないため「保存されたデータなし」として扱われることを、教材自身の言葉で説明する）。
...
- **扱う内容**:
  ...
  5. 本章末時点で、4〜5章のテストコードが以下の呼び出しのみで書き直せることを示すサンプルコード。
     ...
     - FUNC-42
     - FUNC-39（読み込みボタンクリック、COMP-04関数設計書FUNC-39）
     - FUNC-41
...
- 実際の到達経路は以下の3章である。うちFUNC-50はコマンド操作を伴わない概念解説であるため「迷わず到達できる」という要求を損なわない。
  1. FUNC-49（SDK確認・プロジェクト作成・ブラウザインストール・動作確認・サンプルアプリの取得／ビルド／起動確認）
...
**観点**: 教材中のサンプルコード・解説文言に、実在の個人情報・秘密情報が混入する余地がないか。
