...
- **テストレベル**: 単体（関数・クラス単位。COMP-02/03のC#関数、成果物構成の機械的確認）／結合（COMP-01〜03をブラウザ経由で通したE2E。本プロジェクトではPlaywright E2Eが結合・統合を兼ねるため、E2Eは「統合」として扱う）／統合（アプリ全体をサーバープロセスとして起動しブラウザから操作するE2E、および教材ドキュメントの検査）。
...
|---|---|---|---|---|---|---|
| UT-03-01 | パストラバーサル文字列・予約デバイス名・NUL文字を含むuserIdのパス変換 | ファイル名が`[0-9a-f]{64}.json`、基準ディレクトリ直下 | FUNC-19 | REQ-03, CON-05 | 単体 | 仕様/設計 |
...
| UT-03-13 | 書き込み時のI/O異常 | IOExceptionが伝播（success:falseにしない） | FUNC-23, FUNC-25 | REQ-03, NFR-06 | 単体 | 仕様/設計 |
...
| テストID | 内容 | 期待結果 | 対応関数 | 対応要件 | レベル | 由来 |
|---|---|---|---|---|---|---|
| E2E-01 | 入力→送信→メッセージ種別の検証（FUNC-47） | 受理ケースはsuccess、BND-TEXT-OVERLENはerror | FUNC-01, FUNC-03, FUNC-05, FUNC-07, FUNC-09, FUNC-16, FUNC-25, FUNC-27〜40, FUNC-42, FUNC-43〜47 | REQ-01, REQ-02, REQ-03, REQ-08, REQ-09, CON-01, CON-03, CON-04 | 統合 | 仕様/設計 |
| E2E-02 | 送信→再読み込み→読み込み→復元値の検証（FUNC-48） | 受理ケースは入力値どおり復元、拒否ケースはinfoメッセージ | FUNC-02, FUNC-04, FUNC-06, FUNC-08, FUNC-17, FUNC-26, FUNC-27〜42, FUNC-43〜46, FUNC-48 | REQ-04, REQ-05, REQ-06, REQ-08, REQ-09, NFR-07, CON-01, CON-04 | 統合 | 仕様/設計 |
| E2E-03 | 6種類のUIコントロールと注意書きの表示 | 全コントロール表示、ラジオ3件、注意書きに「個人情報」 | （静的HTML） | REQ-01, REQ-07, NFR-01 | 統合 | 仕様/設計 |
| E2E-04 | Cookie未設定での初回リクエスト | lp_user_id（HttpOnly）が発行される | FUNC-10, FUNC-11, FUNC-18, FUNC-39 | REQ-06, CON-05 | 統合 | 仕様/設計 |
...
| テストID | 内容 | 期待結果 | 対応関数 | 対応要件 | レベル | 由来 |
|---|---|---|---|---|---|---|
| UT-XX-01 | ロジック層アセンブリの参照 | Microsoft.AspNetCore.*を参照しない | FUNC-19〜26 | NFR-06, CON-03 | 単体 | 仕様/設計 |
...
- CON-06（Public公開）・CON-07のコミット履歴メタデータ確認は、デプロイ工程のpush前スキャン（`docs/qa_log.md`「Pushスキャン記録」）で確認する。
