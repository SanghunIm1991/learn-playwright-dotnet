...
- **未pushのコミットあり**: 2026-09-24の再レビュー関連コミットを含め`origin/main`より先行している（`git status -sb`で`ahead`を確認）。push前スキャン（`docs/05_publication/publication_manual.md` 4章）を最新コミットまで行い、ユーザーの承認を得てから`git push`する。
