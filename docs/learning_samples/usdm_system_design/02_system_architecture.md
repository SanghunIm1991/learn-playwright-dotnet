...
flowchart TB
    ...
    AE09 -. "起動待ち・直接の要求・応答の差し替え（IF-01）<br/>Cookie発行の確認（IF-02）" .-> AE03
    AE10 -- "直接呼び出し" --> AE05
    AE10 -- "直接呼び出し" --> AE03
    AE10 -- "直接呼び出し" --> AE04
```
...
| AE-09 | E2Eシナリオテスト | サーバーとブラウザの起動・停止、テストごとのブラウザ状態の分離、シナリオの実行と確認。入力・クリック・値の読み取りといった普段の操作はAE-08に任せる。AE-08が扱わない確認（部品の表示と注意書きの文言、Cookieの発行とHttpOnly属性、API応答を差し替えたときの画面の反応、APIへの直接の要求）は自分で行う。サーバーの起動待ちには.NETの`HttpClient`を、それ以外にはPlaywrightの機能を使う | `tests/LearnPlaywright.Tests/Scenarios/FormDataScenarioTests.cs` |
...
通常の利用者はAE-02です。AE-09も、サーバーの起動待ち・JSON以外の形式を拒否することの確認で直接要求を送り、サーバーの異常時（500）や、特殊な値（選択肢にないselect、`radio: null`）を含む応答のときの画面の反応を確かめるために、ブラウザ上で応答を差し替えます。
...
AE-09は、初回の要求でこのCookieが発行され、値が空でなく、HttpOnly属性が付いていることを、ブラウザの状態から直接確かめます（Path・Max-Age・SameSiteの確認は、自動テストに含まれていません）。
...
AE-09は、入力・クリック・値の読み取りをAE-08に任せます。直接行うのは、部品が表示されていること、ラジオボタン（`radio-option`）の個数、注意書き（`notice`）の文言の確認です。
