...
| 方針2: 閉じた環境で動かす | SR-03-01, SR-03-02, SR-03-03, 制約3 | 認証を持たない代わりに、待ち受けをループバックに限り、外から届く経路そのものを減らす |
...
| AE-09 | E2Eシナリオテスト | サーバーとブラウザの起動・停止、テストごとのブラウザ状態の分離、シナリオの実行と確認 | `tests/LearnPlaywright.Tests/Scenarios/FormDataScenarioTests.cs` |
| AE-10 | 単体テスト | AE-03・AE-05の関数を直接呼んで確かめる。AE-05のWeb非依存も確かめる | `tests/LearnPlaywright.UnitTests/` |

### 4.3 依存のルール

- 依存の向きは「画面 → HTTP → ロジック → ファイル」の一方向とし、逆向きの依存は作らない。
- AE-05は.NETの標準ライブラリだけに依存する（AE-10のプロジェクト構成テストで確かめる）。
...
| 発行する条件 | 要求にこのCookieがないとき（保存・読み込みのどちらでも） |
...
### IF-04 画面の取り決め（AE-01 ⇔ AE-02・AE-08）
...
| AD-05 | 他サイトからの送り込みを、「JSON以外は拒否」とSameSite=Laxで防ぐ | 要求ごとの確認用の値（CSRFトークン）を使う | 学習用の小さなシステムでは、トークンの管理よりも仕組みが単純で、ブラウザの標準の性質だけで防げる | SR-03-03 |
...
| SR-03-04 実在の情報を入力させない | ○ | | | | | | | | | |
| SR-04-01 ロジックを単独で検証 | | ○ | ○ | | ○ | | | | | ○ |
| SR-04-02 環境の違いで詰まらない | | | | | | | | | | |

注: SR-04-02は、部品ではなくリポジトリ全体の設定（`global.json`でのSDKの固定、ビルド工程のない画面）で実現しているため、どの部品にも割り当てていません。

### 点検結果

| 観点 | 結果 |
|---|---|
| どの部品にも割り当てのない要求はないか | SR-04-02のみ（上の注の理由による） |
