...
| 仕様 <層の分離> | SP-04-01-01 | 入力検証・保存ファイル用のJSON変換・ファイルの読み書きは、Webの仕組み（ASP.NET Core）に依存しない独立したプロジェクトにまとめること | `Logic/`（`LearnPlaywright.Logic`） |
| 仕様 <分離の確認> | SP-04-01-02 | 上記のプロジェクトがASP.NET Coreのフレームワークを参照していないことを、自動テストで確かめること | `UnitTests/ProjectConstraintTests.cs` |
| 仕様 <HTTP層の単純化> | SP-04-01-03 | HTTP処理は、要求の受け付け条件の確認（形式・大きさ）・識別子の扱い・要求本文と応答のJSON変換・利用者単位の排他・応答コードの決定だけを行うこと。入力検証・保存・読み込みの処理本体はロジックへ委譲し、差し替えられる形で受け取ること | `Server/FormDataApi.cs` PostDeps, GetDeps |
