...
| `examples/replace-text.txt` | 実際に使った「文字列の置き換え」の指示ファイル。消したかった旧記述は伏せ字、置き換え後の表現は実物のまま | 対象（公開される） |
| `examples/commit-callback.py` | 実際に使った「コミットごとの加工」の処理。名義・メールアドレスは伏せ字、処理部分は実物のまま | 対象（公開される） |
| `private/`（同名の2ファイル） | 伏せ字にしていない実物 | **対象外**（`.gitignore`で除外） |

`private/`の2つのファイルには、消したかった記述や旧メールアドレスがそのまま入っています。そのため`.gitignore`で除外し、このPCの中だけに置いています。`git add -f`（除外を無視した強制追加）はしないでください。`git clean -fdx`を実行すると消えるほか、エディタの検索結果には出てきます。不要になったら`private/`ごと削除してください。読んで学ぶ目的なら、`examples/`で足ります。
...
- 今回の`examples/replace-text.txt`（伏せ字版）は、上の1行目の形式（文字列をそのまま指定）で4行あります。書くときに工夫した点は次の3つです。
...
今回の処理は次のとおりです（`examples/commit-callback.py`と同じ。実際の値は伏せています）。
